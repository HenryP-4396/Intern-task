...
class DoubanTop100Crawler(BaseCrawler):
    def __init__(self, db: MySqlHelper) -> None:
        super().__init__(db, sleep_seconds=1)
    ...
    def crawl(self) -> list[dict]:
        ...
        for start in range(0, 100, 25):
            html = self.fetch_html(self.base_url, params={"start": start, "filter": ""})
            movies.extend(self.parse_page(html))
        return movies[:100]
